...
| **User classes** | The legacy `/user/api/get_user_classes` REST path returns HTTP 404. Classes derive from `user_profile.get_profile` → `all_classes`, a `{nid → class dict}` mapping. Each class dict is enriched with `is_ta` (bool) derived from the user's `prof_hash` in `user.status`; enrichment is silently skipped on API failure. |
