    ...
    async def get_post(self, post_id: str) -> Post:
        ...
        try:
            ...
            return Post(
                ...
                bucket_name=raw.get("bucket_name", raw.get("bucket", "")),
                bucket_order=raw.get("bucket_order", 0),
                data=raw.get("data", {}),
                drafts=raw.get("drafts", {}),
                history_size=raw.get("history_size", 0),
                i_edits=raw.get("i_edits", []),
                q_edits=raw.get("q_edits", []),
                s_edits=raw.get("s_edits", []),
                is_bookmarked=raw.get("is_bookmarked", False),
                is_pinned=raw.get("is_pinned", False),
                is_tag_good=raw.get("is_tag_good", False),
                my_favorite=raw.get("my_favorite", False),
                no_answer_followup=raw.get("no_answer_followup", 0),
                num_favorites=raw.get("num_favorites", 0),
                request_instructor=raw.get("request_instructor", 0),
                request_instructor_me=raw.get("request_instructor_me", False),
                t=raw.get("t"),
                tag_good_arr=raw.get("tag_good_arr", []),
            )
